 ...
 *
 * <p><em>Scharr(src, dst, ddepth, xorder, yorder, scale, delta, borderType)</em></p>
 *
 * <p>is equivalent to</p>
 *
 * <p><em>Sobel(src, dst, ddepth, xorder, yorder, CV_SCHARR, scale, delta,
 * borderType).</em></p>
 *
 * @param src Source image.
 * @param dst Destination image of the same size and the same number of channels
 ...
 * channels are numerated from 0 to <code>arrays[0].channels()-1</code>, the
 * second array channels are counted from <code>arrays[0].channels()</code> to
 * <code>arrays[0].channels() + arrays[1].channels()-1</code>, and so on.
 ...
 * histogram. The first array channels are numerated from 0 to <code>arrays[0].channels()-1</code>,
 * the second array channels are counted from <code>arrays[0].channels()</code>
 * to <code>arrays[0].channels() + arrays[1].channels()-1</code>, and so on.
 * @param mask Optional mask. If the matrix is not empty, it must be an 8-bit
 * array of the same size as <code>arrays[i]</code>. The non-zero mask elements
 ...
 * <ul>
 *   <li> <em>src(seed.x, seed.y)- loDiff <= src(x,y) <= src(seed.x, seed.y)+
 * upDiff</em>
 * </ul>
 *
 ...
 * <ul>
 *   <li> <em>src(seed.x, seed.y)_r- loDiff _r <= src(x,y)_r <= src(seed.x,
 * seed.y)_r+ upDiff _r,</em>
 *
 *
 * </ul>
 *
 * <p><em>src(seed.x, seed.y)_g- loDiff _g <= src(x,y)_g <= src(seed.x, seed.y)_g+
 * upDiff _g</em></p>
 *
 * <p>and</p>
 *
 * <p><em>src(seed.x, seed.y)_b- loDiff _b <= src(x,y)_b <= src(seed.x, seed.y)_b+
 * upDiff _b</em></p>
 *
 ...
 * <code>rowKernel</code>. Then, every column of the result is filtered with the
 * 1D kernel <code>columnKernel</code>. The final result shifted by
 * <code>delta</code> is stored in <code>dst</code>.</p>
 *
 * @param src Source image.
 * @param dst Destination image of the same size and the same number of channels
 * as <code>src</code>.
 ...
 * They are determined by <code>thresholdType</code> :</p>
 * <ul>
 *   <li> THRESH_BINARY
 * </ul>
 *
